<div class="flex p-4 {{ $loop->last ? '' : 'border-b border-b-gray-400'}}">
    <div class="mr-4 flex-shrink-0">
        <a href="{{ route('profile', $tweet->user) }}">
            <img class="rounded-full mr-2" width="50px" src="{{ $tweet->user->avatar }}" alt="avatar">
        </a>
    </div>
        ...
        <div class="flex">
            <form action="/tweets/{{ $tweet->id }}/like" method="post">
                ...
                <div class="flex items-center mt-1 mr-4">
                    ...
                    <button type="submit" class="text-xs text-gray-500">{{ $tweet->likess ?: 0 }}</button>
                </div>
            </form>
            ...
            <form action="/tweets/{{ $tweet->id }}/like" method="post">
                ...
                <div class="flex items-center mt-1">
                    ...
                    <button type="submit" class="text-xs text-gray-500">{{ $tweet->dislikess ?: 0 }}</button>
                </div>
            </form>
        </div>
    </div>
